"""Legacy evaluation metrics (preserved for backward compatibility).

This is the original ``evaluation/metrics.py`` module, moved unchanged into the
``evaluation.metrics`` package. It is kept so existing imports such as
``from evaluation.metrics import compute_routing_accuracy`` keep working.

WARNING: ``compute_answer_f1`` here returns *keyword recall*, NOT F1 (see the
docstring/inline note). New code must use
:func:`evaluation.metrics.token_f1.compute_token_f1` instead. This function is
retained only so historical scripts do not break.
"""
...
def compute_answer_f1(
    ...
    """DEPRECATED: returns keyword *recall*, not F1.

    Kept only for backward compatibility with historical scripts. Use
    :func:`evaluation.metrics.token_f1.compute_token_f1` for a correct
    Vietnamese token-level F1.
