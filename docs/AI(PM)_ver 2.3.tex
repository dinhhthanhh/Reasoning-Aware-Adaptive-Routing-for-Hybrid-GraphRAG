...
Experiments on a strict $600$-query Vietnamese legal QA test
...
clarification queries. On a separate constructed ambiguity
benchmark, Stage~1 has clarification F1 $=0.000$ because it is
trained on three retrieval classes and has no clarify label.
Adding Stage~2 increases clarification F1 to $0.684$ with
precision $1.000$, but recall remains $0.519$. After adding
deterministic history-referent resolution and a
severe-ambiguity override policy, a separate $160$-query
conversation stress test reaches clarification F1 $=0.862$
and history-resolution accuracy $=0.850$. These results
support adaptive routing as a practical quality--efficiency
trade-off for Vietnamese Legal Hybrid GraphRAG, while
identifying semantic ambiguity detection as the main remaining
bottleneck.
...
not performed first. The motivation of this work is
therefore to make retrieval strategy selection an explicit
intermediate decision before answer generation. Given a
user query and optional conversation history, the proposed
router first extracts reasoning and ambiguity features, then
uses a fast Stage~1 XGBoost classifier to predict an initial
route, and finally invokes a Stage~2 LLM verifier only for
uncertain, ambiguous, or relation-heavy cases
\cite{chen2016xgboost,jeong2024adaptive}. The main
...
FrugalGPT and RouteLLM demonstrate that routing can reduce
inference cost while maintaining quality when different
models or inference paths have different capabilities and
costs \cite{chen2023frugalgpt,ong2024routellm}. These works
motivate the cost-aware aspect of our router. However, they
mainly focus on routing among model sizes, retrieval depths,
or QA strategies, rather than explicitly routing among dense
retrieval, graph traversal, hybrid reasoning, and
clarification for legal QA.
...
analysis, evaluation protocol, and scope limitations. For
example, MIST presents a dataset together with its generation
pipeline, statistics, quality analysis, and task-specific
metrics for a different modality and problem setting
\cite{vu2026mist}. We follow this reporting style for
Vietnamese legal QA: the present dataset is not an audio
tampering benchmark, but its description is organized around
data sources, construction rules, routing labels, split
...
Dataset
\cite{vu2026mist} &
Reports sources, generation pipeline, statistics, quality
analysis, and scope, but for a different modality and task. &
Dataset \\
...
\node[main, right=of stage1] (trigger) {Trigger check\\$\Gamma(q_t,H)$};
...
\end{tikzpicture}}
\vspace{0.4cm}
\caption{Overview of the proposed Reasoning-aware Adaptive Router for Vietnamese Legal Hybrid GraphRAG. The figure also explains the notation used in the paper: $q_t$ is the current query, $H$ is the conversation history, $\mathbf{f}$ is the extracted feature vector, $\phi_{\mathrm{amb}}$ is the ambiguity score, $\xi$ is the multi-hop score, $(d_1,s_1)$ is the Stage~1 route and confidence, and $d^*$ is the final route. The dashed fallback guard summarizes the implementation policy that retries another retrieval backend when the first answer is uninformative.}
...
the Pháp điển migration and vector-chunk linking. Compared
with the earlier graph snapshot, the most important schema
fix is that Pháp điển article nodes are now exposed as
...
Table~\ref{tab:graph_stats} and Table~\ref{tab:graph_relations}
report the current graph statistics. The \textsc{HAS\_ARTICLE}
...
\renewcommand{\arraystretch}{1.08}
\begin{tabularx}{\columnwidth}{L{2.45cm}L{3.65cm}Y}
\toprule
\textbf{Signal} & \textbf{Condition} & \textbf{Purpose} \\
\midrule
Low confidence &
$s_1 < \tau_{\mathrm{conf}}$ &
Verify uncertain Stage~1 decisions before retrieval. \\
Ambiguity risk &
$b \ge \tau_{\mathrm{force}}$ &
Avoid unsafe retrieval when the legal target is underspecified. \\
Resolved history &
$r_H.\mathrm{status}=\texttt{resolved}$ &
Use the resolved referent to avoid unnecessary clarification. \\
Unresolved history &
$r_H.\mathrm{status}$ is no, irrelevant, or conflicting &
Force verification or clarification for unresolved contextual references. \\
Dense fast path &
High-confidence dense route with low ambiguity and no strong legal signal &
Skip Stage~2 to preserve latency on clear direct-lookup queries. \\
Dense legal signal &
Dense route with legal-reference, graph-keyword, authority, or legal-effect signals inside the confidence ceiling &
Catch dense false negatives that may require graph or hybrid retrieval. \\
Reasoning signal &
Multi-hop, cross-document, high-complexity, or relation-heavy features inside the uncertainty band &
Invoke the verifier for relation-heavy legal questions. \\
...
The routing results support the main claim of adaptive
...
$0.805$ and Macro-F1 $0.780$; this diagnostic is separate
...
($0.739$). However, XGBoost is chosen as the Stage~1
...
\begin{table}[!htbp]
\centering
\caption{Routing classifier comparison on the strict test split ($600$ queries). All classifiers use question text only; MetadataRuleRouter is oracle-style and excluded from comparison. PhoBERT achieves the best Macro-F1 but XGBoost is chosen for latency.}
...
Table~\ref{tab:clarify_results} reports clarification results
on the $234$-query constructed ambiguity benchmark. The most important
finding is that Stage~1 has clarify F1 $=0.000$. This is not
an implementation bug. Stage~1 is trained on the strict
...
therefore cannot architecturally predict clarification. All
clarification behavior in the current system depends on
Stage~2 and the ambiguity trigger.

\begin{table}[!htbp]
\centering
\caption{Clarification results on the constructed ambiguity benchmark.}
...
The full two-stage system improves clarify F1 from $0.000$
to $0.684$. Its clarify precision is $1.000$, meaning that
when the system predicts \texttt{clarify}, the prediction is
correct in this benchmark. However, clarify recall is only
...
shows that the ambiguity detector is incomplete.
...
\end{axis}
\end{tikzpicture}
\caption{Clarification metrics on the ambiguity benchmark.
...
After adding deterministic history-referent resolution, we
evaluate the router on the $160$-query conversation stress
test described in Section~\ref{subsec:dataset_description}.
This diagnostic is reported separately from the strict
end-to-end benchmark because it measures whether the router
uses conversation history correctly, not answer generation
quality. Table~\ref{tab:conversation_stress_summary} compares
the router before and after Phase~3. Clarify F1 improves from
$0.545$ to $0.862$, and history-resolution accuracy reaches
$0.850$. The improvement comes from two complementary
behaviors: resolved history can unblock retrieval, while
absent, irrelevant, or conflicting history can preserve or
force clarification.

\begin{table}[!htbp]
\centering
\caption{Conversation-aware ambiguity stress-test results.}
...
\begin{table}[!htbp]
\centering
\caption{Conversation stress-test category results after Phase~3.}
...
multi-interpretation remains weak on the old template
benchmark because these queries often look answerable without
an explicit contextual pronoun.

\begin{table}[!htbp]
\centering
\caption{Original ambiguity benchmark before and after Phase~3.}
...
$942$ non-Stage~2 cases, while routing with Stage~2 costs
...
predict clarification by design. The current system relies on
Stage~2 for clarification, which improves clarify F1 from
$0.000$ to $0.684$ but still misses many ambiguous queries.
Second, the ambiguity benchmark is synthetic and
template-generated. It is useful for controlled stress
testing, but it should be validated with manually written or
human-reviewed ambiguous legal questions before being treated
as a realistic production benchmark. Third, the ambiguity
benchmark shows that Stage~2 detects surface cues such as
unresolved pronouns and incomplete context, but fails on
semantic ambiguity such as missing legal entities and multiple
valid interpretations. This is the most important limitation
of the current clarification module.
...
routing-only strict sanity run after Phase~3 still produces
$10$ false clarifications out of $600$ queries, so threshold
calibration remains necessary before conversation-aware
policies should replace the strict end-to-end configuration.
...
repeal relations, replacing the XGBoost Stage~1 classifier
with a PhoBERT-based model to improve routing Macro-F1 from
$0.739$ to approximately $0.901$ at the cost of higher
inference latency, applying class-balancing techniques to
address the training-set imbalance toward
\texttt{dense\_retrieval}, and expanding answer-quality
evaluation with human review.
...
The strict $600$-query benchmark shows that adaptive routing
improves answer F1 over both Pure Vector and Pure Graph. The
...
The ambiguity benchmark shows that clarification handling is
the main role of Stage~2. Stage~1 has clarify F1 $=0.000$
because it is trained only on three retrieval labels. Adding
Stage~2 increases clarify F1 to $0.684$ with precision
$1.000$, but recall remains $0.519$. The detailed ambiguity
analysis shows that the current verifier detects unresolved
pronouns and incomplete context, but misses missing-entity
and multi-interpretation ambiguity. These results support the
central design of reasoning-aware adaptive routing while also
identifying the next technical bottleneck for the project:
semantic ambiguity detection in Vietnamese legal QA.

The Phase~3 conversation-aware extension further shows that
history resolution is a useful intermediate signal for legal
QA. On the $160$-query conversation stress test, the router
reaches clarify F1 $=0.862$ and history-resolution accuracy
$=0.850$. The same follow-up query can therefore be routed
differently depending on whether history contains a unique
legal referent. However, resolved-history route selection and
multi-interpretation ambiguity remain open problems, and the
strict routing-only sanity check confirms that false
clarification must still be controlled before updating the
main end-to-end configuration.
